...
app.delete('/remove-player', (req, res) => {
  let matchingPlayerID = 0;
  let matchingTeamID = 0;
  ...
  for (let i = 0; i < teamResults.length; i++) {
    if (teamResults[i].Name === Team) {
      matchingTeamID = teamResults[i].TeamID;

      break; 
    }
 }
 let listOfNames = 'SELECT Name, idPlayer FROM Player;'
 db.query(listOfNames, (err, nameResults) => {
  if (err) {
    console.error('Can\'t get list of names:', err);
    return res.status(500).json({ error: 'Internal server error' });
    }

  // Find the team ID based on the provided team name
  for (let i = 0; i < nameResults.length; i++) {
    if (nameResults[i].Name === Name) {
      matchingPlayerID = nameResults[i].idPlayer;
      break; 
    }
 }
 sql += matchingPlayerID;
 sql += ' AND TeamID = ' + matchingTeamID;
 ...
 console.log(sql);
  ...
  if (err) {
    res.status(200).send('Cannot Remove Player');
    return;
  }
...
app.post('/update-player', (req, res) => {
  let matchingPlayerID = 0;
  let matchingTeamID = 0;
  const { Name, Team, Position , Goals, Assists} = req.body;

  //Create a SQL query to insert player data into the database
  let sql = `UPDATE Player
  SET Goals = ` + Goals + `, Assists = ` + Assists + ` WHERE TeamID = `;
 ...
 sql += matchingTeamID;
 sql += ' AND idPlayer = ' + matchingPlayerID + ";"
 console.log(sql);
  ...
  db.query(sql, (err, result) => {
  if (err) {
    res.status(200).send('Cannot Update Player');
    return;
  }
